...
- **Corrección de Generación Automtica**: Solucionado un problema en el backend donde la autogeneracin de partidos ignoraba a los equipos seleccionados para descansar (UUIDs).
- **Nuevo Modal de Eliminación**: Reemplazo de las alertas nativas del navegador por un modal elegante y personalizado para eliminar partidos. Para usuarios ULTRA, este modal permite elegir entre un borrado total o un borrado conservando el registro financiero en el Reporte IA.
- **Conservación de Cancha**: Al eliminar un partido y mantener su registro financiero, el sistema ahora guarda correctamente el nombre de la cancha donde se jug para su visualizacin en la Agenda Global.
...
Gesti�n de Adeudos y Borrado de Jornadas Autom�ticas.

- **Borrado de Jornadas Autom�ticas**: Implementaci�n de la opci�n para borrar por completo una jornada autogenerada, siempre y cuando ninguno de sus partidos haya finalizado (tenga resultado). Tambi�n se incluy� la capacidad de regenerar los enfrentamientos de la jornada usando equipos distintos.
- **Identificaci�n de Deudores en Agenda Global**: En la vista web de la agenda global, al compartir (PNG) y en el Excel exportado, los equipos que presenten adeudos (balance negativo) aparecer�n con su nombre resaltado en color rojo, mejorando la visibilidad para el administrador y los due�os.
- **Ocultamiento de Saldos en Cero**: En el Resumen General Financiero y sus respectivas exportaciones, los equipos y �rbitros cuyo balance llegue exactamente a  dejar�n de mostrarse en la tabla de deudores, limpiando el reporte de informaci�n innecesaria.
- **Informaci�n de Descansos**: Al generar autom�ticamente una jornada en ligas con cantidad impar de equipos, el sistema ahora indica visualmente qu� equipo fue asignado para descansar esa fecha.
- **Resaltado de Enfrentamientos Previos**: En el modal de agendar partidos manuales, el �ltimo enfrentamiento contra cada equipo se resalta en color rojo para una identificaci�n visual r�pida de los cruces previos.
