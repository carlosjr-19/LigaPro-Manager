...
def update_db():
    ...
    print("Checking 'court_id' column in 'matches' table...")
    try:
        ...
        if 'court_id' not in columns:
            print("Adding 'court_id' column to 'matches' table...")
        ...
        print(f"Error altering matches table: {e}")

    # 3. Create default court for existing leagues and assign matches
    ...
    try:
        ...
        for league_id, league_name in leagues:
            ...
            if court_count == 0:
                ...
                # Assign all existing matches of this league to this court
                cursor.execute(
                    "UPDATE matches SET court_id = ? WHERE league_id = ? AND court_id IS NULL",
                    (default_court_id, league_id)
                )
                print(f"  -> Assigned matches to default court.")
